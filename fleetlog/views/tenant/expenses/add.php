...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <h1 class="text-2xl font-bold text-slate-800">Add Expense to Vehicle: <?php echo htmlspecialchars($vehicle['license_plate']); ?></h1>
    </div>
</div>

<div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
    <form action="/tenant/expenses/add/<?php echo $vehicle['id']; ?>" method="POST" class="space-y-6">
        ...
        <div class="pt-4 border-t border-slate-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="odometer_at_expense" value="<?php echo $vehicle['current_odometer']; ?>" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all font-mono text-slate-500" readonly title="Updated automatically via driver trips">
                ...
                <div id="service_reminder_container" class="bg-blue-50 p-4 rounded-xl border border-blue-100 hidden">
                    ...
                    <input type="number" name="next_service_km" id="next_service_km" placeholder="e.g. <?php echo $vehicle['current_odometer'] + 15000; ?>" class="w-full px-4 py-3 rounded-xl border border-blue-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-300 outline-none transition-all font-mono">
                    <p class="text-xs text-blue-600 mt-2">Optional. Use this to set a reminder for the next oil change or revision. You will be alerted when the vehicle nears this kilometer limit.</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-4 pt-4 border-t border-slate-100">
            <a href="/tenant/vehicles" class="px-6 py-3 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition-all">Cancel</a>
...
<script>
    // Logic to only show the "Next Service KM" field when "maintenance" or "consumable" is selected
