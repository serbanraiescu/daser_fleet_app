            ...
            <?php foreach ($templates as $tmpl): ?>
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <form action="/admin/sms/template-update" method="POST" class="p-6">
                        ...
                        <div class="mb-4">
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Pas Alertă (Zile înainte de expirare)</label>
                            <input type="text" name="alert_days" value="<?php echo htmlspecialchars($tmpl['alert_days'] ?? '30,7,3,1'); ?>" 
                                   class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-mono text-sm"
                                   placeholder="Ex: 30, 15, 7, 3, 1">
                            <p class="mt-1 text-[10px] text-slate-400">Separă prin virgulă numărul de zile pentru fiecare reminder.</p>
                        </div>
                        <div class="mb-4">
